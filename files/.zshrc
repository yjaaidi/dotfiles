...
# Fix invalid pyenv configuration warning
eval "$(pyenv init -)"
...
#source ~/.oh-my-zsh/custom/plugins/yarn-completion/yarn-completion.plugin.zsh
...
alias limbo='while :; do git fetch; [ $(git rev-parse HEAD) != $(git rev-parse @{u}) ] && git pull --rebase --autostash && git push; done'
alias lsoflisten="lsof -iTCP -sTCP:LISTEN -P -n"
# alias join='/usr/bin/python2 "/System/Library/Automator/Combine PDF Pages.action/Contents/Resources/join.py" --verbose $@'
